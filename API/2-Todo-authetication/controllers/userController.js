...
const register = async (req, res, next)=>{
    ...
    gmail = gmail?.trim().toLowerCase();
      ...
      console.log("Trimmed gmail:", gmail);
  // Validate input
  if (!username || !password || !gmail) {
    ...
    try {
      ...
      res
        .status(201)
        .json({ newUser, message: "User registeration suceessful" });
    } catch (error) {
        next(error);
        console.log(error);
    }
...
const login = async (req, res, next) => {
    ...
    const user = await userModel.findOne({ gmail });
        if (!user) {
    return res.status(400).json({ message: "Gmail not found, please register with us" });
        }
